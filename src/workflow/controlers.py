...
from workflow import settings, utils, data, dag, argo, dd
...
def sync(minioClient, job_name, job_url, repo_code_path, repo_data_path='data'):
    ...
    dependencies, _ = data.get_changed_files(minioClient,
                                             job_name, latest_commit,
                                             join('src', job_name))

    logging.warning(dependencies)

    changed_tasks = [task for task in dependencies.keys() if dd.has_changed(task["image"])]
    subdag = dag.get_subdag_of_changed_images(dependencies, changed_tasks)
    tasks = dag.get_merged_tasks([subdag])
