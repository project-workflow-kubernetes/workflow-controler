...
def register_job(minioClient, job_name, job_url, repo_code_path,
    ...
    lookup = get_lookup_paths(dependencies, commit_hash,
                                tmp_path, repo_code_path, repo_data_path)

    bucket = minioClient.Bucket(job_name)
    print('bucket')
    tmp_to_persistent(bucket, job_name, lookup)
...
def get_persistent_state(minioClient, job_name, job_url, prefix_tmp_path=settings.VOLUME_PATH):
